...
static int adl_gpio_direction_output(struct gpio_chip *gc, unsigned int nr, int value)
{
    u8 dir, cap = 0;
    int ret;
    ...
    if (cap != 0) {
        ...
        dir = dir & ~(1 << nr_ext);
        adl_bmc_ec_write_device(ADL_BMC_OFS_GPIO_DIR_EXT, &dir, 1, EC_REGION_1);
        adl_gpio_set(gc, nr_ext, value);
    } else {
        ret = adl_bmc_ec_read_device(ADL_BMC_OFS_GPIO_DIR, &dir, 1, EC_REGION_1);
        dir = dir & ~(1 << nr);
        adl_bmc_ec_write_device(ADL_BMC_OFS_GPIO_DIR, &dir, 1, EC_REGION_1);
        adl_gpio_set(gc, nr, value);
    }
...
{
    flag = 0;  // Reset the flag to allow reopening
    return 0;
}
...
static int adl_ec_gpio_remove(struct platform_device *pdev)
{
    ...
    cdev_del(&cdev);
    unregister_chrdev_region(devdrv, 1); // Correct function call
    return 0;
}
